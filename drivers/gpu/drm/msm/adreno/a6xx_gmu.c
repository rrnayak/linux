...
static int a6xx_gmu_fw_start(struct a6xx_gmu *gmu, unsigned int state)
{
	...
	if (state == GMU_WARM_BOOT) {
		...
		/* Load the firmware in case it hasn't been done yet */
...
static void a6xx_gmu_disable_clocks(struct a6xx_gmu *gmu)
{
	int i;

	for (i = 0; i < gmu->nr_clocks; i++)
		clk_disable_unprepare(gmu->clocks[i]);
}

static void a6xx_gmu_enable_clocks(struct a6xx_gmu *gmu)
{
	int i;
	...
	for (i = 0; i < gmu->nr_clocks; i++)
		clk_prepare_enable(gmu->clocks[i]);
}
...
int a6xx_gmu_reset(struct a6xx_gpu *a6xx_gpu)
{
	...
	a6xx_gmu_disable_clocks(gmu);
	pm_runtime_put_sync(gmu->dev);

	/* Re-enable the resources */
	pm_runtime_get_sync(gmu->dev);
	a6xx_gmu_enable_clocks(gmu);
...
static int a6xx_gmu_clocks_probe(struct a6xx_gmu *gmu)
{
	...
	gmu->nr_clocks = of_property_count_strings(gmu->dev->of_node,
		"clock-names");

	if (gmu->nr_clocks < 1) {
		dev_err(gmu->dev, "No clocks have been defined\n");
		gmu->nr_clocks = 0;
		return 0;
	}

	gmu->clocks = devm_kcalloc(gmu->dev, sizeof(struct clk *),
		gmu->nr_clocks, GFP_KERNEL);
	if (!gmu->clocks) {
		gmu->nr_clocks = 0;
		return -ENOMEM;
	}

	of_property_for_each_string(gmu->dev->of_node, "clock-names", prop,
		name) {
		gmu->clocks[i] = devm_clk_get(gmu->dev, name);
		if (IS_ERR(gmu->clocks[i])) {
			int ret = PTR_ERR(gmu->clocks[i]);

			dev_err(gmu->dev, "Unable to allocate clock %s: %d\n",
				name, ret);
		}

		if (strcmp(name, "gmu") || strcmp(name, "gmu_clk"))
			gmu->core_clk = gmu->clocks[i];
